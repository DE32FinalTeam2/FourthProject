...
def collect_metrics():
    while True:
        ...
        # 네트워크 대역폭 사용률 계산 및 기록
        network_query = '(rate(container_network_receive_bytes_total[5m]) + rate(container_network_transmit_bytes_total[5m])) * 100'
        ...
        with cpu_placeholder.container():
            st.line_chart(df["CPU Usage (%)"], use_container_width=True)

        with memory_placeholder.container():
            st.line_chart(df["Memory Usage (%)"], use_container_width=True)

        with network_placeholder.container():
            st.line_chart(df["Network Bandwidth Usage (%)"], use_container_width=True)

        time.sleep(1)  # 1초마다 갱신

if __name__ == "__main__":
    print("Starting to collect system usage metrics...")
    collect_metrics()
